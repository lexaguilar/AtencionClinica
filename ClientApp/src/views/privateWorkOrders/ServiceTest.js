...
const ServiceTest = ({ customerId, user, open, followId }) => {
    ...
    useEffect(() => {
        http(`followsprivate/${followId}/getServicesSent`).asGet().then(resp => {
            setIds({
                id: 0,
                sendTestId: 0
            });
                ...
                <div className="mr-10" style={{ width: '30%' }}>
                    <DataGrid
                        ...
                        dataSource={ids.sendTestId == 0 ? [] : store({
                            uri: {
                                get: `followsprivate/${followId}/getServicesSent/${ids.sendTestId}`
                            }
                        })}
                        showBorders={true}
                        showRowLines={true}
...
const GridTests = ({ id, serviceId }) => {
    return (
        <DataGrid
            ...
            dataSource={store({
                uri: {
                    get: `followsprivate/${id}/getServicesSent/${serviceId}/Details`,
                    insert: `followsprivate/${id}/getServicesSent/${serviceId}/Details`
                }
            })}
            showBorders={true}
            showRowLines={true}
...
const GridBaarTests = ({ id, serviceId }) => {
    return (
        <DataGrid
            ...
            dataSource={store({
                uri: {
                    get: `followsprivate/${id}/getServicesSent/${serviceId}/Details/baar`,
                    insert: `followsprivate/${id}/getServicesSent/${serviceId}/Details/baar`
                }
            })}
            showBorders={true}
            showRowLines={true}
